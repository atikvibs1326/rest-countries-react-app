            ...
            {country.length > 0 && country.map((item) => (
                <div key={item.population} className="grid grid-cols-1 gap-8 md:grid-cols-2 md:place-items-center md:h-screen">
                    ...
                    <article>
                        ...
                        dark:text-gray-400">
                            <li>Captial:{item.capital[0]}</li>
                            <li>Population:{item.population.toLocaleString()}</li>
                            <li>Region:{item.region}</li>
                            <li>SubRegion:{item.subregion}</li>
                            <li>Currencies:{currencies(item)}</li>
                        </ul>
                        ...
                        dark:text-white">Borders:</h3>
                                <ul className="flex flex-wrap items-start justify-start gap-2 dark:text-white">
                                    {item.borders.map((border, index) => (
                                        <li
                                            key={index}
                                            className="bg-white p-2 rounded text-xs tracking-wide shadow
                                dark:bg-gray-800 dark:text-gray-400">{border}</li>
                        ...
                        text-gray-700 hover:bg-gray-200 transition-all duration-200 dark:bg-gray-800 
